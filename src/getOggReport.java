...
public class getOggReport {
   ...
   private static void getRetrieveServices(String depName, JSONObject ojb) throws IOException, ParseException, SQLException 
   {
	   String host=null;
	   ...
	   for (int i = 0; i < getInput.length(); i++) {
    	    ...
    	    oggHttp=oggUrl+"/services/v2/deployments/"+depName+"/services/"+name;
	 	    ...
	 	    if (name.contains("adminsrvr")) {
	 		  ...
	 		  JSONArray getItems = (JSONArray) response.get("items");
	 		 ...
	 		 getItems = (JSONArray) response.get("items");
	 		 if (getItems.length()>0) {
	 			  System.out.println("\tReplicat -->");
   ...
   private static void getlistProcess(JSONObject ojb,String type) throws IOException, ParseException, SQLException 
   {
       ...
       //System.out.println("type: "+type+ " "+ listExtract.length() + " -- " + listExtract);
       for (int i = 0; i < listExtract.length(); i++) {
			...
			if (type.equals("EXTRACT")) { 
				//System.out.println("\r\n");
				listProcess.put(name.toString(), "EXTRACTS ------>" +name); 
				JSONObject result=httpOgg(httpUrlService+"extracts/"+name.toString()+"/info/reports",username,password,"GET","");
				//System.out.println(httpUrlService+"replicats/"+name.toString()+"/info/reports"); 
			...
			if (type.equals("REPLICAT")) { 
				//System.out.println("\r\n");
				listProcess.put(name.toString(), "REPLICATS ------>" +name); 
   ...
   private static Map<String, String> getDeployment(JSONObject ojb) throws IOException, ParseException, SQLException 
   {
       ...
       for (int i = 0; i < getInput.length(); i++) {
    		...
    		if (!depName.contains("ServiceManager")) {
  	    		...
  	    		System.out.println("Deploy:"+depName);
